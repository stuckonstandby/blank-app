...
def main():
    ...
    **Monthly consumption (kWh)** is entered manually below.
    """)

    # 1. Load Historical Rate Data (so we can limit date range)
    csv_path = "/workspaces/blank-app/historical_data_AB_ele.csv"
    try:
    ...
    st.header("Monthly Consumption Patterns (kWh)")
    MONTH_NAMES = [
    ...
    # 8. Aggregate monthly data (average rates in cents/kWh)
    ...
    # 9. Calculate monthly costs (in cents)
    monthly_cost_utility = []
    monthly_cost_client = []

    for _, row in monthly_rates.iterrows():
        month_datetime = pd.Timestamp(row["year_month"])
        reg_rate = row["regulated_rate"]  # (cents/kWh)
        whl_rate = row["wholesale_rate"]  # (cents/kWh)

        mnum = month_datetime.month
        month_consumption = consumption[mnum]  # (kWh)

        # Utility Cost (Regulated Rate)
        cost_utility = month_consumption * reg_rate  # in cents
        ...
        if use_hedge and hedge_start_ts and (hedge_start_ts <= month_datetime < hedge_end_ts):
            ...
            cost_hedged = hedged_volume * (hedge_fixed_rate + admin_fee)  # cents
            cost_floating = floating_volume * (whl_rate + admin_fee)      # cents
            cost_client = cost_hedged + cost_floating
        else:
            cost_client = month_consumption * (whl_rate + admin_fee)      # cents

        monthly_cost_utility.append(cost_utility)
        monthly_cost_client.append(cost_client)

    total_utility_cents = sum(monthly_cost_utility)
    total_client_cents = sum(monthly_cost_client)

    # Convert to dollars
    utility_dollars = total_utility_cents / 100
    client_dollars = total_client_cents / 100
